...
WHERE orders.order_date >= '2018-06-01' AND orders.order_date <= '2018-06-30'
...
SELECT AVG(cost) -- Calculate the average monthly cost before September
FROM monthly_cost
...
WITH user_monthly_activity AS (
	SELECT DISTINCT
		DATE_TRUNC('month', order_date) :: DATE AS delivr_month,
		user_id
	FROM orders) -- If active in given month.

SELECT
-- Calculate the MoM retention rates
	previous.delivr_month,
	ROUND(
		COUNT(current.user_id) :: NUMERIC /
		COUNT(previous.user_id),
		2) AS retention_rate
FROM user_monthly_activity AS previous
LEFT JOIN user_monthly_activity AS current
-- Fill in the user and month join conditions
	ON previous.user_id = current.user_id AND previous.delivr_month = (current.delivr_month - INTERVAL '1 month')
...
-- Calculate the average orders per user
	ROUND(orders :: NUMERIC / COALESCE(users,1), 2) AS arpu
FROM kpi;

-- ----------------------------------------
-- CALCULATING VALUES FOR HISTOGRAM, BUCKETING
-- ----------------------------------------
...
-- Fill in the bucketing conditions
	...
	ELSE 'High-revenue users' END AS revenue_group,
...
-- Write the conditions for the three buckets
	...
	ELSE 'High-orders users' END AS order_group,
...
-- ----------------------------------------
-- PIVOTING
-- CREATE EXTENSION IF NOT EXISTS tablefunc, CROSSTAB($$ 'Query' $$)
-- ----------------------------------------
...
SELECT * FROM CROSSTAB($$
	...
	JOIN orders
		ON meals.meal_id = orders.meal_id
	WHERE user_id IN (0, 1, 2, 3, 4) AND order_date < '2018-09-01'

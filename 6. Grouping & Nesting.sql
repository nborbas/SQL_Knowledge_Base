-- ----------------------------------------
-- CASE STUDY - BRINGING EVERYTHING TOGETHER
-- ----------------------------------------
...
SELECT
	MIN(rating) AS min_rating, -- Calculate the minimum rating
	MAX(rating) AS max_rating, -- Calculate the maximum rating
	AVG(rating) AS avg_rating, -- Calculate the average rating
	COUNT(rating) AS number_ratings -- Count the number of ratings
FROM renting
...
-- ----------------------------------------
-- SIMPLE SQL QUERIES (INCL. SUBQUERIES) FOR DECISION MAKING
-- ----------------------------------------
...
WHERE date_renting > '2017-12-31' AND date_renting < '2019-01-01';


SELECT
	m.title, -- Create a list of movie titles and actor names
	a.name
FROM actsin AS ai
	LEFT JOIN movies AS m
...
GROUP BY movie_id
HAVING
	AVG(rating) >  -- Of movies with rating above average
	(SELECT AVG(rating)
	FROM renting);
...
-- ----------------------------------------
-- OLAP SQL QUERIES (ONLINE ANALYTICAL PROCESSING)
-- CUBE() - extract content from Pivot table, ROLLUP()  - subtotals, GROUPING SETS((..),(..),()) - subtotals over defined levels = UNION of GROUP BY results
-- ----------------------------------------

SELECT gender, country, COUNT(*) -- Extract information of a pivot table of gender and country for the number of customers
FROM customers
